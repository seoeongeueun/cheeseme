                ...
                <div className="rightContent">
                    ...
                    {openBookmark ? <BookmarkPosts allPosts={allPosts} setOpenBookmark={setOpenBookmark} userId={userId} friendId={friendId} onSetFriendId={onSetFriendId}/>
                        ...
                        {(!edit && !openBookmark) && <div className='rightFooter'>
                                    ...
                                    <div className='pageSliderButtons'>
                                        <button onClick={() => {value <= 1 && onChangeDate(date - 86400000); setValue(value < 1 ? 0 : value-1);}}><ArrowBackIosNewRoundedIcon sx={{fontSize: '1.7rem'}}/></button>
                                        <span style={{marginBottom: '2rem'}}>{currentFriendName !== '' ? `@ ${currentFriendName}` : userId ? `@ ${name}` : 'Login Required'} </span>
                                        <button onClick={() => {value >= allPosts?.length && onChangeDate(date + 86400000); setValue(value > allPosts?.length ? allPosts?.length : value + 1);}}><ArrowForwardIosRoundedIcon sx={{fontSize: '1.7rem'}}/></button>
                                    </div>
                                </div>}
                    </div>}
            </div>
