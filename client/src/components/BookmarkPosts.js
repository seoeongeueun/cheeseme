        ...
        <GridLines className="grid-area" cellWidth={60} strokeWidth={1} strokeWidth2={1} cellWidth2={12} lineColor2={"#e1e1e1"} lineColor={"#d4d4d4"}>
            <div className='bookmarkInnerBorder'> 
                <div className="rightBookmarkPosts">
                    <p style={{textAlign: "center", fontSize: '3rem'}}>Bookmarked Posts</p>
                    <div className='foundPostsCategory'>
                        {foundPosts?.length > 0 ? foundPosts.map((p, index) => (
                            <div className='foundSearchItem' onClick={() => handleClick(p.date)}>
                                ...
                                <div className='foundContent'>
                                    {p.title.length <= 0 ? <span className='foundContentTitle' style={{width: 'max-content', backgroundColor: colorCode[index%colorCode.length]}}>No Title</span> : <span className='foundContentTitle' style={{width: 'max-content', backgroundColor: colorCode[index%colorCode.length]}}>{p.title}</span>}
