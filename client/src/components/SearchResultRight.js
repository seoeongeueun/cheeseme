      ...
      for (let i = 0; i < 5; i++) {
        if (indexes[i] + val.length >= arr.length) {
          if (indexes[i] - 20 <= 0) {
            ...
            result =
              result +
              ('...' + arr.substring(indexes[i] - 40, indexes[i] + val.length));
          }
        } else if (indexes[i] === 0) {
          if (indexes[i] + val.length + 40 >= arr.length) {
            ...
            result =
              result +
              (arr.substring(indexes[i], indexes[i] + val.length + 40) + '...');
          }
        } else {
          result =
            result +
            ('...' +
              arr.substring(indexes[i] - 20, indexes[i] + val.length + 20) +
              '...');
        }
      }
      return result;
        ...
        if (indexes[i] + val.length >= arr.length) {
          if (indexes[i] - 50 <= 0) {
            ...
            result =
              result +
              ('...' +
                arr.substring(indexes[i] - 100, indexes[i] + val.length));
          }
        } else if (indexes[i] === 0) {
          if (indexes[i] + val.length + 100 >= arr.length) {
            ...
            result =
              result +
              (arr.substring(indexes[i], indexes[i] + val.length + 100) +
                '...');
          }
        } else {
          if (indexes[i] - 50 <= 0) {
            result =
              result +
              (arr.substring(0, indexes[i] + val.length + 100) + '...');
          } else {
            result =
              result +
              ('...' +
      ...
      >
        <div className="leftContentSearch">
          <div className="marker" onClick={() => handleClickHome()}>
            <span style={{ margin: '0rem 1rem 1.5rem 0rem' }}>
              <HomeSharpIcon sx={{ fontSize: '1.7rem' }} />
            </span>
          </div>
          <p style={{ textAlign: 'center' }}>
            Searching Entries for <b>{keyword}</b>
          </p>
          <div className="searchOption">
            <div
              className="searchOptionHeader"
              onClick={() => handleSearchOption()}
            >
              <span style={{ marginRight: '1rem' }}>Search by</span>
              {!showOption && (
                <button>
                  <span style={{ fontSize: '2rem', color: '#f73939' }}>
                    {searchBy}
                  </span>
                </button>
              )}
            ...
            <div className="searchOptionContent">
              <div className="searchOptionDrop">
                {showOption && (
                  <span onClick={() => handleSearchBy('Content')}>
                    {' '}
                    Content
                  </span>
                )}
                {showOption && (
                  <span onClick={() => handleSearchBy('Title')}> Title</span>
                )}
              </div>
            </div>
          </div>
                ...
                foundPosts.map((note, index) => (
                  ...
                  >
                    ...
                    <div className="foundContent">
                      ...
                      {searchBy === 'Title' ? (
                        <span>
                          {removeTags(note.text).substring(0, 100).length >= 100
                            ? removeTags(note.text).substring(0, 100) + '...'
                            : removeTags(note.text).substring(0, 100)}
                        </span>
                      ) : (
                        <Highlighter
