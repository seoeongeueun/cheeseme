        ...
        .then((res) => {
          ...
          if (n)
            setFoundTodos(
              res?.data.filter((t) =>
                t.goals.find((g) =>
                  g.text.toLowerCase().includes(keyword.toLowerCase()),
                ),
              ),
            );
          return;
      ...
      for (let i = 0; i < indexes.length; i++) {
        if (indexes[i] + val.length >= arr.length) {
          if (indexes[i] - 50 <= 0) {
            ...
            result =
              result +
              ('...' + arr.substring(indexes[i] - 50, indexes[i] + val.length));
          }
        } else if (indexes[i] === 0) {
          if (indexes[i] + val.length + 50 >= arr.length) {
            ...
            result =
              result +
              (arr.substring(indexes[i], indexes[i] + val.length + 50) + '...');
          }
        } else {
          if (indexes[i] - 25 <= 0) {
            if (indexes[i] + val.length + 50 >= arr.length)
              result = result + arr.substring(0, indexes[i] + val.length + 50);
            else
              result =
                result +
                (arr.substring(0, indexes[i] + val.length + 50) + '...');
          } else {
            if (indexes[i] + val.length + 15 >= arr.length)
              result =
                result +
                ('...' +
                  arr.substring(indexes[i] - 35, indexes[i] + val.length + 15));
            else
              result =
                result +
                ('...' +
                  arr.substring(indexes[i] - 25, indexes[i] + val.length + 25) +
                  '...');
          }
        }
      }
      ...
      for (let i = 0; i < indexes.length; i++) {
        if (indexes[i] + val.length >= arr.length) {
          if (indexes[i] - 80 <= 0) {
            ...
            result =
              result +
              ('...' + arr.substring(indexes[i] - 80, indexes[i] + val.length));
          }
        } else if (indexes[i] === 0) {
          if (indexes[i] + val.length + 80 >= arr.length) {
            ...
            result =
              result +
              (arr.substring(indexes[i], indexes[i] + val.length + 80) + '...');
          }
        } else {
          if (indexes[i] - 40 <= 0) {
            if (indexes[i] + val.length + 80 >= arr.length)
              result = result + arr.substring(0, indexes[i] + val.length + 80);
            else
              result =
                result +
                (arr.substring(0, indexes[i] + val.length + 80) + '...');
          } else {
            if (indexes[i] + val.length + 20 >= arr.length)
              result =
                result +
                ('...' +
                  arr.substring(indexes[i] - 60, indexes[i] + val.length + 20));
            else
              result =
                result +
                ('...' +
                  arr.substring(indexes[i] - 40, indexes[i] + val.length + 40) +
                  '...');
          }
        }
      }
      ...
      >
        <div className="leftContentSearch">
          <p style={{ textAlign: 'center' }}>
            Searching Widgets for <b>{keyword}</b>
          </p>
                ...
                foundNotes.map((note, index) => (
                  ...
                  >
                    ...
                    <Highlighter
                      highlightTag={'b'}
                      searchWords={[keyword]}
                      textToHighlight={cutString(note.text, keyword)}
                      highlightStyle={{
          ...
          <div className="foundWidgetCategory">
            ...
            <div className="foundSearchItems">
              {foundTodos?.length > 0 ? (
                foundTodos.map((todo, index) => (
                  <div
                    key={index}
                    className="foundSearchItem"
                    onClick={() => handleClick(todo.date)}
                  >
                    <span className="foundDate">
                    ...
                    {todo.goals.map((g, index) => (
                      <div key={index} className="foundTodos">
                        <Highlighter
                          highlightTag={'b'}
                          searchWords={[keyword]}
                          textToHighlight={g.text}
                          highlightStyle={{
                            backgroundColor:
