...
function Reminder({ move, onCreate, onToggle, userId }) {
  ...
  useEffect(() => {
    if (userId) {
      ...
      setReminders([
        {
          title: 'This is guest mode',
          detail:
            'You can test things out as guest but your changes will not be saved!',
          check: false,
          _id: 0,
        },
        {
          title: 'debit card expiration date 06/23',
          detail: 'card renewal required',
          check: true,
          _id: 1,
        },
      ]);
  ...
  const handleAdd = () => {
    ...
    if (userId) {
      ...
      setReminders([
        ...reminders,
        { title: '', detail: '', check: false, _id: reminders.length },
      ]);
    }
  };
  ...
  return (
    <div className="reminderWidget">
      ...
      <div className="reminderHeader">
        <span style={{ marginLeft: '0.5rem', marginBottom: '0.1rem' }}>
          Reminder
        </span>
        <div className="reminderHeaderButtons">
          ...
          {editMode ? (
            <button onClick={() => handleSave()}>
              <CheckRoundedIcon sx={{ fontSize: '1.5rem', color: '#f73939' }} />
            </button>
          ) : (
            <button onClick={() => setEditMode(true)}>
              <RemoveRoundedIcon sx={{ fontSize: '1.5rem' }} />
            </button>
          )}
        </div>
      </div>
      <div
        className="reminderContent"
        id="reminderContent"
        style={{
          overflowY: show ? 'scroll' : 'hidden',
          maxHeight: show ? '17rem' : '',
        }}
      >
        {reminders?.length > 0 &&
          reminders.map((r, i) => (
            <div className="reminderItem" key={r._id}>
              <div className="reminderTitle">
                <button
                  style={{ backgroundColor: colorCode[i % colorCode.length] }}
                >
                  {editMode ? (
                    <>
                      <input
                        ...
                        value={r.title}
                        onChange={(e) => handleEditReminder(i, e.target.value)}
                      />
                      <ClearRoundedIcon
                        onClick={() => handleDelete(i)}
                        sx={{ fontSize: '1.2rem', color: '#f73939' }}
                      />
                    </>
                  ) : (
                    <label
                      style={{
                        textDecorationLine: r.check ? 'line-through' : 'none',
                        cursor: 'pointer',
                        textDecorationThickness: '1px',
                      }}
                      onClick={() =>
                        userId
                          ? handleCheckTitle(r._id)
                          : handleCheckTitleGuest(i)
                      }
                    >
                      {r.title}
                    </label>
                ...
                <button
                  ...
                  style={{
                    width: '10%',
                    backgroundColor: colorCode[i % colorCode.length],
                  }}
                >
                  {show ? (
                    <ArrowDropUpIcon sx={{ fontSize: '1.5rem' }} />
                  ) : (
                    <ArrowDropDownIcon sx={{ fontSize: '1.7rem' }} />
                  )}
                </button>
              </div>
              {show && !editMode && (
                <div
                  className="reminderDetail"
                  onClick={() => setShow(false)}
                  style={{
                    backgroundColor: colorCodeLight[i % colorCodeLight.length],
                  }}
                >
                  <p style={{ fontSize: '1.9rem', whiteSpace: 'pre-wrap' }}>
                    {r.detail}
                  </p>
                </div>
              )}
              {show && editMode && (
                ...
                >
                  <textarea
                    style={{ marginLeft: '0.6rem', fontSize: '1.7rem' }}
                    value={r.detail}
                    onChange={(e) =>
                      handleEditDetail(i, r.title, e.target.value)
                    }
                  />
                </div>
              )}
