...
function DisplaySettings(props) {
  ...
  return (
    <div className="settingsbox">
      <span className="settingsboxTitle" style={{ marginTop: '-0.2rem' }}>
        Style Settings
      </span>
        ...
        <div className="checkboxButton">
          {props.plain ? (
            <CheckBoxRoundedIcon
              className="checkbox"
              sx={{ fontSize: '1.5rem' }}
              onClick={() => handlePlainClick(false)}
            />
          ) : (
            <CheckBoxOutlineBlankRoundedIcon
              className="checkbox"
              sx={{ fontSize: '1.5rem' }}
              onClick={() => handlePlainClick(true)}
            />
          )}
        ...
        <div className="checkboxButton">
          {props.grid ? (
            <CheckBoxRoundedIcon
              className="checkbox"
              sx={{ fontSize: '1.5rem' }}
              onClick={() => handleGridClick(false)}
            />
          ) : (
            <CheckBoxOutlineBlankRoundedIcon
              className="checkbox"
              sx={{ fontSize: '1.5rem' }}
              onClick={() => handleGridClick(true)}
            />
          )}
          <label>Grid</label>
        </div>
